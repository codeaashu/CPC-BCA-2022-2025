...
if (isset($_POST['submit'])) {
    ...
    if (!preg_match("/^[6-9][0-9]{9}$/", $mobileno)) {
        $errors[] = "Mobile number must start with 6-9 and be exactly 10 digits.";
    }
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    nameField.addEventListener('blur', function () {
        ...
        if (value.length > 0 && !namePattern.test(value)) {
            alert("Name must contain only letters and spaces (minimum 3 characters).");
    ...
    mobileField.addEventListener('blur', function () {
        ...
        if (value.length > 0 && !mobilePattern.test(value)) {
